...
func TestJWTAuthRepository_Create(t *testing.T) {
...
func TestJWTAuthRepository_GetSubject(t *testing.T) {
...
func TestJWTAuthRepository_RefreshToken(t *testing.T) {
    ctrl := gomock.NewController(t)
    defer ctrl.Finish()
    logger := mock_log.NewMockLogger(ctrl)
    mockClock := mock_clock.NewMockClock(ctrl)
...
func TestJWTAuthRepository_Validate(t *testing.T) {
    privatePEM := []byte("-----BEGIN RSA PRIVATE KEY-----\\ngjOIscTm3jUVsz8bCkXrVlFsWRVCnvQwKx788Awq6mdw\n-----END RSA PRIVATE KEY-----")
    publicPEM := []byte("-----BEGIN PUBLIC KEY-----\nMIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQChTrU+r2uTQPQOxBCwKVAM0AJP\nnB4MEh+MggX5lkrGOPtzBglzV2DF+ydVknJoqmUFbnczXJsAFaaaXCYm7N/kOcww\nsL+q7oKBKbiJYtrYGr7uoJrOJ1SIWq/RnvkWpGGqth6SvryEB742l0WAiG3nxWY+\nm1EXClDQU1sAa4LMeQIDAQAB\n-----END PUBLIC KEY-----")
    publicKey, _ := jwt.ParseRSAPublicKeyFromPEM(publicPEM)
    privateKey, _ := jwt.ParseRSAPrivateKeyFromPEM(privatePEM)
...
func TestJWTAuthRepository_createPair(t *testing.T) {
    ctrl := gomock.NewController(t)
    defer ctrl.Finish()
    logger := mock_log.NewMockLogger(ctrl)
    mockClock := mock_clock.NewMockClock(ctrl)
...
func TestJWTAuthRepository_keyFunc(t *testing.T) {
    ctrl := gomock.NewController(t)
    defer ctrl.Finish()
    logger := mock_log.NewMockLogger(ctrl)
    mockClock := mock_clock.NewMockClock(ctrl)
...
func TestNewJWTAuthRepository(t *testing.T) {
    ctrl := gomock.NewController(t)
    defer ctrl.Finish()
    logger := mock_log.NewMockLogger(ctrl)
    mockClock := mock_clock.NewMockClock(ctrl)
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            if got := NewJWTAuthRepository(tt.args.config, tt.args.clock, tt.args.logger); !reflect.DeepEqual(got, tt.want) {
                t.Errorf("NewJWTAuthRepository() = %v, want %v", got, tt.want)
            }
        })
    }
}

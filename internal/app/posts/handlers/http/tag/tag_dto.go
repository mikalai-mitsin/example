...
func NewTagDeleteDTO(r *http.Request) (TagDeleteDTO, error) {
	del := TagDeleteDTO{}
	if err := render.DecodeJSON(r.Body, &del); err != nil {
		return TagDeleteDTO{}, err
	}
	del.ID = uuid.MustParse(chi.URLParam(r, "id"))
	return del, nil
}
func (dto TagDeleteDTO) toEntity() (entities.TagDelete, error) {
	del := entities.TagDelete{ID: dto.ID}
	return del, nil
}

...
type LikeDeleteDTO struct {
	ID uuid.UUID `json:"id"`
}

func NewLikeDeleteDTO(r *http.Request) (LikeDeleteDTO, error) {
	del := LikeDeleteDTO{}
	if err := render.DecodeJSON(r.Body, &del); err != nil {
		return LikeDeleteDTO{}, err
	}
	del.ID = uuid.MustParse(chi.URLParam(r, "id"))
	return del, nil
}
func (dto LikeDeleteDTO) toEntity() (entities.LikeDelete, error) {
	del := entities.LikeDelete{ID: dto.ID}
	return del, nil
}

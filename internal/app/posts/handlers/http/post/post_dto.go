...
func NewPostDeleteDTO(r *http.Request) (PostDeleteDTO, error) {
	del := PostDeleteDTO{}
	if err := render.DecodeJSON(r.Body, &del); err != nil {
		return PostDeleteDTO{}, err
	}
	del.ID = uuid.MustParse(chi.URLParam(r, "id"))
	return del, nil
}
func (dto PostDeleteDTO) toEntity() (entities.PostDelete, error) {
	del := entities.PostDelete{ID: dto.ID}
	return del, nil
}

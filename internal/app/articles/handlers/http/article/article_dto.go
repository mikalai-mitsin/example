...
func NewArticleDeleteDTO(r *http.Request) (ArticleDeleteDTO, error) {
	del := ArticleDeleteDTO{}
	if err := render.DecodeJSON(r.Body, &del); err != nil {
		return ArticleDeleteDTO{}, err
	}
	del.ID = uuid.MustParse(chi.URLParam(r, "id"))
	return del, nil
}
func (dto ArticleDeleteDTO) toEntity() (entities.ArticleDelete, error) {
	del := entities.ArticleDelete{ID: dto.ID}
	return del, nil
}

		...
		},
		{
			...
			setup: func() {
				mockUserUseCase.
					EXPECT().
					Create(ctx, gomock.Any()).
					Return(entities.User{}, errs.NewUnexpectedBehaviorError("usecase error")).
					Times(1)
			},
			fields: fields{
				UnimplementedUserServiceServer: examplepb.UnimplementedUserServiceServer{},
		...
		},
		{
			...
			setup: func() {
				mockUserUseCase.EXPECT().Get(ctx, user.ID).
					Return(entities.User{}, errs.NewUnexpectedBehaviorError("i error")).
					Times(1)
			},
			fields: fields{
				UnimplementedUserServiceServer: examplepb.UnimplementedUserServiceServer{},

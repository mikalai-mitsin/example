...
import (
	...
	mockEntities "github.com/mikalai-mitsin/example/internal/app/user/entities/mock"
...
func TestUserRepository_Create(t *testing.T) {
	...
	query := "INSERT INTO public.users (created_at,updated_at,first_name,last_name,password,email,group_id) VALUES ($1,$2,$3,$4,$5,$6,$7)"
...
func TestUserRepository_List(t *testing.T) {
	...
	for i := 0; i < faker.New().IntBetween(2, 20); i++ {
		listUsers = append(listUsers, mockEntities.NewUser(t))
	}
...
func TestUserRepository_Update(t *testing.T) {
	...
	mockLogger := NewMocklogger(ctrl)
	user := mockEntities.NewUser(t)
	query := `UPDATE public.users SET created_at = $1, updated_at = $2, first_name = $3, last_name = $4, password = $5, email = $6, group_id = $7 WHERE id = $8`
	ctx := context.Background()
	type fields struct {
		database *sqlx.DB
		logger   logger
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := r.Update(tt.args.ctx, tt.args.user)
			assert.ErrorIs(t, err, tt.wantErr)
		})
	}
}

		...
		},
		{
			...
			setup: func() {
				mockUserRepository.EXPECT().
					Get(ctx, user.ID).
					Return(entities.User{}, errs.NewEntityNotFoundError())
			},
			fields: fields{
				userRepository: mockUserRepository,
				logger:         mockLogger,
...
func TestUserService_List(t *testing.T) {
	...
	filter := mockEntities.NewUserFilter(t)
	type fields struct {
		userRepository userRepository
		logger         logger
	}
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func() {
				mockUserRepository.EXPECT().
					List(ctx, filter).
					Return([]entities.User{}, errs.NewUnexpectedBehaviorError("test error"))
			},
			fields: fields{
				userRepository: mockUserRepository,
				logger:         mockLogger,
...
func TestUserService_Create(t *testing.T) {
	...
	mockUUID := NewMockuuidGenerator(ctrl)
	ctx := context.Background()
	create := mockEntities.NewUserCreate(t)
	now := time.Now().UTC()
	type fields struct {
		userRepository userRepository
		clock          clock
...
func TestUserService_Update(t *testing.T) {
	...
	user := mockEntities.NewUser(t)
	mockClock := NewMockclock(ctrl)
	update := mockEntities.NewUserUpdate(t)
	now := time.Now().UTC()
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() {
				...
				mockUserRepository.EXPECT().
					Update(ctx, updatedUser).Return(nil)
			},
			fields: fields{
				userRepository: mockUserRepository,
				clock:          mockClock,

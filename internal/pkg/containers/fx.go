...
import (
	...
	"github.com/mikalai-mitsin/example/internal/app/arch"
	"github.com/mikalai-mitsin/example/internal/app/auth"
	"github.com/mikalai-mitsin/example/internal/app/session"
...
}, context.Background, configs.ParseConfig, clock.NewClock, uuid.NewUUIDv4Generator, postgres.NewDatabase, postgres.NewMigrateManager, grpc.NewServer, uptrace.NewProvider, auth.NewApp, session.NewApp, arch.NewApp, user.NewApp), fx.Invoke(func(lifecycle fx.Lifecycle, server *uptrace.Provider, config *configs.Config) {
